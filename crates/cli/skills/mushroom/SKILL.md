...
The `learn` pass — `/mushroom learn <path>` — turns prose (design docs, ADRs, READMEs) into `Concept` nodes the graph can keep honest. Per run: **at most 20 documents**. Per document: **at most 5 concepts**. One concept is one idea somebody could ask about by name.
...
`tools/list` shows eleven: the eight above plus `query` (Cypher, read or write), `ingest_json` (bulk-load a JSON array) and `stats`. Thirteen more are callable but unlisted — `create_rule`, `explain`, `explain_association`, `neighborhood`, `node_info`, `node_edges`, `upsert_entity`, `find_similar`, `hybrid_search`, `node_history`, `edge_history`, `was_linked`, `rename_node` — and `{{BIN}} mcp '{{DB_PATH}}' --all-tools` lists them with the schemas documenting their arguments. For a restricted audience pass `mask` on `query` (and `find_similar`): it is an **allow-list**, so only the listed keys are visible, every other node is omitted, and writes are rejected while it is set. This MCP server has **no auth** and those masks are cooperative, so never present one as a security boundary — real access control is the HTTP server's role tokens (`{{BIN}} serve --role-token`). Never invent graph contents: if a tool returns empty say so, and if a call fails show the error verbatim. `serve` browses the same store — `{{BIN}} serve '{{DB_PATH}}'` puts the live explorer at `http://127.0.0.1:8080` — and `mushroomdb doctor` checks the install.

---

More: `{{BIN}} --help` · [docs](https://github.com/MatthewSherlin/mushroomdb/tree/main/docs/site)
