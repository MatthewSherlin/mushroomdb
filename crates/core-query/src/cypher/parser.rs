...
impl<'a> Parser<'a> {
    ...
    fn set_clause(&mut self) -> Result<SetClause, String> {
        ...
        let value = match self.peek() {
            ...
            _ => {
                return Err(self.err(
                    "expected literal, $parameter, or arithmetic expression as SET value",
                ))
            }
        };
...
LIMIT 10";
    ...
    fn is_not_null_parses_on_prop() {
        ...
        assert_eq!(
            q.where_expr,
            Some(Expr::IsNotNull(prop("a", "x"))),
        );
    }

    #[test]
    fn is_null_on_var() {
        let q = parse_src("MATCH (a) OPTIONAL MATCH (a)-[:T]->(b) WITH a, b WHERE b IS NULL RETURN a").unwrap();
    ...
    fn arith_div_in_where() {
        ...
        assert!(matches!(
            q.where_expr,
            Some(Expr::Cmp {
                lhs: Operand::BinArith { op: ArithOp::Div, .. },
                ..
            })
        ));
    }
    ...
    fn create_return_parses_node_var() {
        ...
        use crate::cypher::ast::{WriteStatement, RetVal};
    ...
    fn create_return_prop_with_alias() {
        ...
        match stmt {
            WriteStatement::Create(s) => {
                ...
                assert_eq!(returns[0].value, RetVal::Prop { var: "n".into(), field: "id".into() });
    ...
    fn is_write_tokens_still_true_for_create_return() {
        ...
        assert!(is_write_tokens(&toks), "CREATE...RETURN must still be classified as write");
    }
