...
fn empty_db_returns_no_suggestions() {
    ...
    assert!(
        ...
        "expected empty, got: {suggestions:?}"
    );
}
...
fn determinism_same_seed_identical_output() {
    ...
    for (a, b) in r1.iter().zip(r2.iter()) {
        ...
        assert_eq!(
            ...
            "rationale must be identical (same seed)"
        );
        assert_eq!(
            a.examples.len(),
            b.examples.len(),
        ...
        for ((src1, dst1, score1), (src2, dst2, score2)) in a.examples.iter().zip(b.examples.iter())
        {
...
fn different_seeds_may_differ() {
    ...
    assert_eq!(
        r1.len(),
        r2.len(),
        "same data, different seeds: candidate count must match"
    );
}
...
fn global_budget_fires_and_sets_truncated() {
    ...
    let partial: SuggestReport = db.suggest_rules_with_config(&config_zero, SUGGEST_DEFAULT_SEED);
    assert!(
        partial.truncated,
