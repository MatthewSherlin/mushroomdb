...
use core_api::{json_to_value, AutoFk, Dir, GraphError, IngestOptions, ResultSet, SharedDb, Value};
...
fn result_set_json(rs: &ResultSet) -> Js {
...
fn params_from_json(v: Option<&Js>) -> Result<BTreeMap<String, Value>, String> {
    let Some(v) = v else {
