...
# below.
#
# The cache is one line of text per resolved path under $CLAUDE_PLUGIN_DATA (or
# ~/.mushroomdb), named for the version that wrote it, so a plugin upgrade
# resolves afresh rather than running the old copy. Every failure falls through
# to the next rung, ending at the plain `npx` form, which is slower and always
# correct.
#
...
# read_cache <file> — the single line in it, if the file is readable.
read_cache() { [ -f "$1" ] && cat "$1" 2>/dev/null; }

# remember <file> <path> — best effort; a cache that cannot be written just
# means the next hook resolves again.
remember() { (mkdir -p "$CACHE_DIR" && printf '%s\n' "$2" >"$1") 2>/dev/null; }
