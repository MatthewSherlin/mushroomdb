...
The `learn` pass — `/mushroom:mushroom learn <path>` — turns prose (docs, ADRs, READMEs) into `Concept` nodes. Per run **at most 20 documents**, per document **at most 5 concepts**.
...
`tools/list` follows the store: one built by `ingest-git` shows three — `explore`, `query` (Cypher, read or write) and `stats` — any other store shows eleven. All 25 are served either way; `npx -y mushroomdb@0.6.2 mcp <db> --all-tools` lists the rest with their schemas, `ingest_json` among them. **Never create a rule silently:** *propose* `create_rule`, show the predicate and the edges it would derive, and wait for approval. When `ingest_json` skips a field with `ambiguous target labels`, declare one KeyMatch rule per target label instead. `mask` on `query` (and `find_similar`) is an **allow-list**: only listed keys are visible, and writes are rejected while set. This server has **no auth** and masks are cooperative — never present one as a security boundary; real access control is `serve --role-token`.
