...
| Cypher write subset v1 | CREATE, MATCH…SET, MATCH…DELETE (manual edges only), and MERGE (single-key match-or-create) are supported. SET RHS must be a literal; combined MATCH…SET…RETURN is rejected; multi-statement transactions are not supported. CREATE and MATCH…SET/DELETE produce one WAL Batch frame (one fsync); MERGE-create produces one raw InsertNode frame (also one fsync); MERGE-match (node exists) produces no frame. |
