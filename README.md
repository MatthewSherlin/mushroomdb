...
| `node_edges` | Every edge on one node, grouped by edge type, with the rule and score behind each |
| `neighborhood` | Everything within N hops of a node, optionally one edge type only |
| `edges_at` | The edges a node had at one commit — the graph as it was, not as it is |
| `what_if` | The derived edges a property change would lose and gain, computed without writing anything |
