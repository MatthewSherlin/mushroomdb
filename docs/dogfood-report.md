...
| **Big-3 unanswered** — no matcher edges at 100k | **Big-3 metro/industry slice: p50=581 µs, mean_matches=500** (500T×500C, all 3 rules fire, no cap needed at 250k pairs). Answers the 5-second complaint in a focused bucket. | Full-graph Big-3 = empty intersection (1M cap at 70k×20k = 0.07% coverage). Awaits derived-edge persistence (Roadmap #1) to remove the cold-start penalty that blocks raising caps. |
