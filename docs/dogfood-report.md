# Case study: replacing hand-maintained matching edges with declared rules

**Date:** 2026-08-19  
**Machine:** Apple M4 Pro, 24 GiB RAM, macOS 15.7.3 arm64  
**Scope:** End-to-end validation against a synthesized matching-domain graph (100k nodes, 1536-dim embeddings). Sources: `dogfood/results/scale-100k.md`, `dogfood/ui_run.md`, `dogfood/test_fixtures.py`, `dogfood/test_scale_small.py`.

> **Caveat (applies throughout):** All numbers are from a local embedded process (`mushroomdb.GraphDb`, not HTTP) on Apple silicon with **synthetic hash-chain embeddings**. The workload is a representative matching schema (Talent / Company / Job nodes, six rule kinds) built to validate the engine end-to-end — it is **not** a benchmark against any specific production deployment (networked, sharded, real embeddings). The comparison is structural, not a benchmark replacement.

---

## Background: the hand-rolled approach

A common pattern in matching systems: maintain scored edges between people (Talent) and organizations (Company / Job) by running batch scoring scripts that iterate over all pairs, compute field-level and semantic similarity, and write the resulting edges. When a node property changes, the stale edges must be identified and replaced — a retraction step that is easy to miss and expensive to get right. At production scale (70k Talent × 20k Company), a na&iuml;ve Cartesian re-score ran for over 8 hours and required expert knowledge of retraction semantics to avoid edge drift.

The representative workload here replaced that approach with six declared matching rules that the engine evaluates and maintains automatically.

---

## 1. Semantics — Six Rule Kinds vs Hand-Written Matcher Edges

### Result: PASS on all exercised fixture pairs

Ten rule instances (six semantic kinds) were validated against hand-derived expected sets on the representative fixture set (5 Talent / 2 Company / 2 Job / 7 User nodes). Every exact-set assertion, score pin, and negative-case check passed (`dogfood/test_fixtures.py`, `test_ingest_rules_derived_edges_and_explain`).
...
**Gap 1 — 'both'-industry 0.8 score:** `field_equal(industry)` in these fixtures fires binary 1.0 only. An alternate scoring convention weights the 'both' industry value at 0.8 rather than 1.0 for an exact match; that value is absent from the fixture set (`test_fixtures.py`: `"# field_equal(industry) — binary 1.0. 'both' is absent from these fixtures."`). The 0.8 composition branch is unexercised.
...
### 2e. Anti-overclaim: the Big-3 µs number is not a 5s-query replacement

The Big-3 p50=5.7 µs is three empty `neighbors` calls — no `INDUSTRY_ALIGNMENT`, `SPECIALTY_MATCH`, or `LOCATION_FIT` edges exist at 100k because the backfill was not executed. **This number does not demonstrate that mushroomdb beats a 5-second production matcher at 100k.** The honest claim is: at 2k scale (all matcher rules live, 1,228,422 derived edges), the `node_edges`+`neighbors` API path operates in µs; the backfill to reach that state is where the cost lives, and that cost is blocked by the cartesian wall at 100k. Source: `dogfood/results/scale-100k.md`, "Incremental / Big-3 / explain."
...
| F4 | Label-chip count is 2× query LIMIT due to depth-1 auto-expansion. Potentially surprising with dense matcher nodes. | **Low / Informational** | Accept / document expected behavior |
...
| M4 | **UI F2 — Dense hub add-to-canvas block** | Depth-1 auto-expansion of a 400-edge hub blocks UI > 5 s. At high-fanout density this would be the default behavior for any well-connected node. | UI plan: cap auto-expand depth at 0 (add the root node only) or paginate the expansion; expose "expand" as an explicit user action for dense nodes. |

### Vector-scale strategy (owner decision required)

The 5k ScanAll probe extrapolates to **80.3 hours at 100k** — `semantic_match` cannot ship at this scale in its current O(n²) ScanAll form. Three options with the available data:
...
| **C — Document dims/size limits as known limitation** | Ship semantic_match with ScanAll; document maximum supported scale in the release notes | Zero engine change | `semantic_match` is unusable at >~5k nodes. The representative workload requires it at 100k+. |
...
| 'both'-industry 0.8 score unvalidated | Composition branch absent from fixtures; no evidence it fires. Document as unverified. |
...
Plan 11 (T1–T4) shipped four targeted engine + bindings changes. The re-run at the same 100k scale (seed=20260819) confirmed:
...
- **Cold-start is the new top finding.** Every open() re-derives all rules. Adding a second VectorSimilar rule doubles cold-start time. This is the roadmap blocker for deployment.
...
- **For the WAL-only path**, the bottleneck is IVF-Flat ANN index rebuild (~8 min at 100k × 1536 dim).
  The V4 snapshot path eliminates this entirely. Use `snapshot()` on
