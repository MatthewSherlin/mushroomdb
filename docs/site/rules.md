...
rule**: the beam widens, doubling from the default width (400) up to a ceiling of
4,096, until it has something to conclude.
...
- the 4,096 ceiling reached with the worst hit still at or above `min`, so a
  cluster denser than the ceiling costs a scan and never costs recall;
...
|---|---|---|
| Candidates | HNSW, beam widened until its worst hit is below `min` | HNSW approximate k-NN, one pass at k |
| Scores | Exact, from the stored vectors | Exact, from the stored vectors |
| Edge recall | floor 0.98 asserted, 1.000 measured (5k/dim 1536, fixed-seed probe); exhaustive below the default beam width | min 0.90, mean 0.998 (5k/dim 1536, fixed-seed probe) |
...
Measured at 5k nodes, dim 1536 (release, fixed-seed probe): creating the exact
rule takes 269 s through the index against 142 s for the full scan
`MUSHROOMDB_VECTOR_SCAN=1` restores. At this size the one-time graph build is
larger than the whole quadratic scan it removes, and the index path wins on
creation time only above it; what it buys at 5k is that every later write
re-derives from a beam instead of a full scan. Approximate backfill time is
substantially faster still — the IVF-Flat-era measurement was ~17 s; HNSW timing
at this scale is not separately published (flag: unsure).
...
same two graphs an approximate rule does — one per side — and each holds its own
copy of every vector it indexes as `f64`, so a rule whose src and dst labels are
the same stores the corpus twice. At 5,000 × 1,536 dims that is about 61 MB per
side plus the adjacency lists (M₀ = 128 neighbours at layer 0). Both graphs are
written into the snapshot so that reopening adopts them instead of rebuilding,
which is the same trade the approximate mode has always made, and
`MUSHROOMDB_VECTOR_SCAN=1` does not release it: a session under the variable
still loads and re-persists the graph, so unsetting it later costs nothing. A
rule created *while* the variable is set builds no graph at all, and the first
open without the variable builds one from the node scan (logged, with the vector
count).
