...
rule**: the beam widens — doubling from the default width up to a ceiling of
4,096 — until the worst hit it returns falls below `min`, at which point no node
outside the beam can qualify. A rule over fewer vectors than the default beam
width never walks the graph at all, because a beam that wide is the whole index:
small rules stay exhaustive.
