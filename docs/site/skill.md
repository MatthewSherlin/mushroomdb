...
thirteen on any other — that all 25 stay callable either way and
...
| `--always-load` | **Off by default.** Writes `"alwaysLoad": true` on the `mcpServers.mushroomdb` entry, so the host keeps the server's tools in context instead of deferring them until something asks. Claude Code's `.mcp.json` only — a Cursor or Codex registration has no equivalent. Re-running `install` without the flag takes the key back off; `disable` and `enable` preserve it. |
