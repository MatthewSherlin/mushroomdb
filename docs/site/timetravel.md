...
| `wal.genesis` | first archive, no prior truncation | empty marker; signals that the archive chain covers genesis |
| `wal.truncated` | first `keep_wal=false` snapshot | write-once empty marker; persists across sessions |

`wal.truncated` is intentionally never deleted once written.

**Upgrade note:** `wal.truncated` is a new file introduced in v0.2. It is
written the first time a store that was previously using standard (truncating)
`snapshot()` calls takes a snapshot under v0.2+. The file is harmless and
requires no action. Its presence is what prevents a later archiving session
from incorrectly claiming a complete genesis chain when WAL history has a gap.
...
- `wal.genesis` marker is present (first archive was created before any
  truncating snapshot)
- `wal.floor` is 0 or absent (no archives have been pruned)
- `wal.truncated` is absent (no prior truncating snapshot before archiving began)
