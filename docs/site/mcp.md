...
| `stats` | Return live node, edge, and rule counts, plus `namespaces` — every namespace with a live node and its count. Pass `role` or `namespace` to be told about those namespaces only; the store-wide counts beside them are unchanged. |
